<template>
  <div class="budget-card">
    ...
    <q-dialog v-model="showAddConfirmDialog">
      <q-card style="min-width: 350px">
        <q-card-section>
          <div class="text-h6">Save Annual Budget?</div>
        </q-card-section>

        <q-card-section class="q-pt-none">
          Are you sure you want to save this annual budget?
        </q-card-section>
...
const addBudgetLabel = computed(() => {
  return $q.screen.gt.sm ? "ADD BUDGET" : "";
});
...
const columns = [
  ...
  {
    name: "medicine",
    label: "Medicine Budget",
    field: "medicine_budget",
    align: "center",
  },
  {
    name: "lab",
    label: "Laboratory Budget",
    field: "laboratory_budget",
    align: "center",
  },
  {
    name: "hospital",
    label: "Hospital Budget",
    field: "hospital_budget",
    align: "center",
  },
];
...
const openAddBudgetDialog = () => {
  ...
  const existingBudget = rows.value.find(
    (budget) => budget.year === currentYear,
  );

  if (existingBudget) {
    // If budget exists, show warning instead of opening form

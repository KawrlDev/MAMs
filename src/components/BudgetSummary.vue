<template>
  ...
  <div class="budget-table table-scroll">
    <q-table :rows="summaryRows" :columns="columns" row-key="type" flat bordered :pagination.sync="pagination">
    ...
    <q-table :rows="tabangRows" :columns="columns2" row-key="type" flat bordered :pagination.sync="pagination">
...
const summaryRows = computed(() => {
  ...
  allPatients.value.forEach(p => {
    const month = new Date(p.date).toLocaleString('en-US', { month: 'short' }).toLowerCase()
    const category = p.category
    if (!summary[category]) return

    summary[category].bene[month]++
    summary[category].amount[month] += Number(p.issuedAmount || 0)
  })
...
const tabangRows = ref([
  { category: 'Medicine', budget: 100000, sb: 2, totRel: 50000, remBal: 50000, pax: 120 },
  { category: 'Laboratory', budget: 50000, sb: 1, totRel: 20000, remBal: 30000, pax: 80 },
  { category: 'Hospital Bill', budget: 150000, sb: 3, totRel: 90000, remBal: 60000, pax: 200 },
])
